...
int getNoun(string in)
{
    ...
    if (noun == "I" || noun == "Me" || noun == "me" || noun == "My" || noun == "my")
        cout << "Noun is 1p" << endl;
    else
        cout << "Noun is 3p" << endl;

    nounLength = nounLength + noun.length() + 1;
    return nounLength;
}

string getAction(string in, int del)
{
    string action;
    in.erase(0, del);
    int l = in.length();
...
void evalSentence(string in)
{
    ...
    else
    {
        ...
        noun = getNoun(in);
        action = getAction(in, noun);
        verbTense = verb_tense(action);
        cout << "Verb is: " << verbTense << endl;
        for (int i = 0; i < length; i++)
        {
            word = theWord(in);
